...
### How encoding works at inference (using that dict)

Given `"the"`:

1. Pre-tokenize → chunk `["the"]`
2. Convert to bytes → `[b't', b'h', b'e']`
3. Find the adjacent pair with the lowest rank (= earliest merge = most common):
   - `(b't', b'h')` → rank 256
   - `(b'h', b'e')` → rank 257
   - Merge `(b't', b'h')` first → `[b'th', b'e']`
4. Continue: `(b'th', b'e')` → rank 258 → merge → `[b'the']`
5. No more pairs. Result: token id **258**

This greedy priority-queue approach is what tiktoken implements efficiently in Rust.
